...
// Define a "Full On" frame (all 96 bits set to 1)
  const uint32_t allOn[] = { 0xffffffff, 0xffffffff, 0xffffffff };
  // Define a "Full Off" frame (all 96 bits set to 0)
  const uint32_t allOff[] = { 0x0, 0x0, 0x0 };

  // Flash the entire matrix 5 times
  for (int i = 0; i < 5; i++) {
  ...
  // Flash the entire matrix 1 times
  for (int i = 0; i < 1; i++) {
    matrix.loadFrame(allOn);
    delay(1000);
    matrix.loadFrame(allOff);
